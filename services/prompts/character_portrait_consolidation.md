...
- Do not introduce new names during consolidation. Every canonical `name` must come from the provisional roster or from explicit known/project hints.
- Remove entries with no portrait pages and no visual description unless they are a directly supported real name from the provisional roster.
...
- Do not invent new facts or names that are not reasonably supported by the provisional roster. Never import unrelated franchise casts.
